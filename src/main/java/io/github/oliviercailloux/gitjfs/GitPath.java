...
/**
 * A git path has an optional root component and a (possibly empty) sequence of names (strings). It
 * is also associated to a git file system.
 *
 * <h2>Corresponding commit</h2>
 * <p>
 * The root component, if it is present, represents a commit. It consists in either a git reference
 * (a string which must start with <code>refs/</code>, such as <code>refs/heads/main</code>) or a
 * commit id (represented as an {@link ObjectId}).
 * <p>
 ...
 * <h2>Internal path</h2>
 * <p>
 * The sequence of names represents a path inside a given commit. Each name is a string that does
 * not contain any slash. Names are not empty, except possibly the first name element when it is
 * alone (in the special case of an empty path). If a git path has no root component, then its
 * sequence of names is non empty.
 * <p>
 * A git path is <em>empty</em> iff it has no root component and a single name element which is the
 * empty string. An empty path implicitly refers to the root of the branch <code>main</code>.
 * </p>
 *
 * <h2>String form</h2>
 * <p>
 * The string form of a path consists in the string form of its root component, if it has one,
 * followed by its internal path.
 * </p>
 * <ul>
 * <li>The string form of a root component is <code>/gitref/</code>, where <code>gitref</code> is a
 * git reference; or <code>/sha1/</code>, where <code>sha1</code> is a commit id.</li>
 * <li>Its internal path is a string that starts with a slash iff the path is absolute, and is
 * composed of the names that constitute its sequence of names, separated by slashes.</li>
 * </ul>
 *
 * <h2>Possible cases</h2>
 * <p>
 * It follows from these rules that an instance of this class matches exactly one of these two cases
 * (each admitting a special case).
 * </p>
 * <ul>
 * <li>It has no root component. Equivalently, its string form contains no leading slash.
 * Equivalently, its string form contains no two consecutive slashes. Equivalently, it is a relative
 * path. It implies that its sequence of names is not empty. An example of string form is
 * {@code "some/path"}.
 * <ul>
 * <li>As a special case, its sequence of names may consist in a unique empty name. Equivalently, it
 * is an empty path. Equivalently, its string form is {@code ""}. Such a path implicitly refers to
 * the branch <code>main</code> and the root directory in that branch.</li>
 * </ul>
 * </li>
 * <li>It has a root component. Equivalently, its string form contains a leading slash.
 * Equivalently, its string form contains two consecutive slashes exactly once. Equivalently, it is
 * an absolute path. Implies that its sequence of names contain no empty name. An example of string
 * form is {@code "/refs/heads/main//some/path"}.
 * <ul>
 * <li>As a special case, it may consist in a root component only, in other words, have an empty
 * sequence of names. Equivalently, its string form ends with two slashes. An example of string form
 * is {@code "/refs/heads/main//"}.</li>
 * </ul>
 * </li>
 * </ul>
 *
 ...
 * <p>
 * The string form of the root component starts and ends with a slash, and contains more slashes iff
 * it is a git reference. It has typically the form <code>/refs/category/someref/</code>, where
 * <code>category</code> is <code>tags</code>, <code>heads</code> or <code>remotes</code>, but may
 * have <a href="https://git-scm.com/docs/git-check-ref-format">other</a>
 * <a href="https://stackoverflow.com/a/47208574/">forms</a>. This class requires that the git
 * reference starts with <code>refs/</code>, does not end with <code>/</code> and does not contain
 * <code>//</code> or <code>\</code> (git also imposes these restrictions on git references).
 * </p>
 * <h3>Rationale</h3>
 * <p>
 * The special git reference <code>HEAD</code> is not accepted for simplification: <code>HEAD</code>
 * may be a reference to a reference, which introduces annoying exceptions. E.g. that a GitPathRoot
 * is either a ref (a pointer to a commit) starting with refs/ or a commit id would not be true any
 * more if it could also be <code>HEAD</code>. Another, weaker, reason for refusing
 * <code>HEAD</code> is that it makes sense mainly with respect to a
 * <a href="https://git-scm.com/docs/gitglossary#def_working_tree">worktree</a>, whereas this
 * library is designed to read directly from the git directory, independently of what happens in the
 * worktree. This prevents, for example, deciding that the default path of a git file system is the
 * ref or commit pointed by <code>HEAD</code>. This reason is weaker because bare repositories
 * <a href="https://stackoverflow.com/q/3301956/859604">use HEAD</a> to indicate their default
 * branch, showing that HEAD is not solely for use within a worktree.
 * <p>
 * A {@link Ref} is not accepted as an input because a {@code Ref} has an object id which does not
 * update, whereas this object considers a git reference as referring to commit ids dynamically.
 * Also, {@code Ref} is more general than what is called here a git ref.
 * <p>
 * The fact that the path <code>/somegitref//</code> is considered as a root component only, thus
 * with an empty sequence of names, can appear surprising. Note first that slash is a path
 * separator, thus cannot be part of a name. The only other possible choice is thus to consider that
 * <code>/someref//</code> contains a sequence of name of one element, being the empty string. An
 * advantage of this choice is that the sequence of names would be never empty (thereby easing
 * usage) and that it feels like a natural generalization of the case of <code>""</code>, a path
 * also containing one element being the empty string. But from the wording of
 * {@link Path#getFileName()}, it seems like that method should return null iff the path is a root
 * component only, and that what it should return is distinct from the root component. Note that the
 ...
 * C:\ as a root component only, and returns {@code null} on {@code getFileName()}. (And I believe
 * that also under Windows, <code>\</code> is considered a root component only, equivalently, an
 * empty sequence of names.) For sure, under Linux, <code>/</code> is a root component only. Thus,
 * to behave similarly, we have to return {@code null} to {@code getFileName()} on the path
 * <code>/someref//</code>, hence, to treat it as a root component only. (This choice would also
 * break the nice fact that the internal path in a git path behaves like the sequence of names in a
 * linux path, as <code>/</code> under Linux is an empty sequence.)
 * <p>
 * The Java FS API requires that relative paths be {@link Path#toAbsolutePath() automatically
 * convertible} to absolute paths. This is arguably not very natural for git, as a git repository
 * has no concept of a default branch. To comply with the requirement I have opted for the
 * (probably) most commonly used first branch name, namely, {@code main}. Alternatively I could have
 * looked automatically for the ref <a href=
 * "https://superuser.com/questions/1718677#comment2652531_1718677">/refs/remotes/origin/</a> but
 * this only introduces more complexity and requires another arguable choice, as someone’s remote
 * may be named differently than {@code origin}. Whenever you need to convert a relative path to an
 * absolute path, I advice to use an explicit absolute path (using explicity the branch {@code main}
 * if desired) and resolve your relative path against this explicit absolute path.
 * <p>
 * Equality is driven by the {@link Path#equals(Object)} contract, which requires equality of the
 * file system.
 */
public interface GitPath extends Path {
   ...
   * path. Otherwise, this method returns a path with a root component referring to the
   * <code>main</code> branch.
   *
   * <p>
   * This method does not access the underlying file system and requires no specific permission.
   *
   */
  @Override
   ...
   */
  @Override
  GitPath toRealPath(LinkOption... options)
   ...
   *
   * <p>
   * This method returns {@code true} iff the given object is a git path, their git file systems are
   * equal, and the paths have equal root components (or they are both absent) and internal paths.
   * The internal paths are compared in a case-sensitive way (conforming to the Linux concept of
   * path equality). Equivalently, two git paths are equal iff they are associated to the same git
   * file system and have the same {@link GitPath string forms}.
   *
   * <p>
   * This method does not access the file system and the files are not required to exist.
   *
   */
  @Override
   ...
   *
   * <p>
   * If this path was created by converting a path string using the {@link FileSystem#getPath
   * getPath} method then the path string returned by this method may differ from the original
   * String used to create the path.
   *
   */
  @Override
  String toString();
