 ...
 * <ul>
 * <li>A git ref is a string that starts with refs/, does not contain "//" or "\" and does not end
 * with "/".</li>
 * <li>A commit id is a SHA-1.</li>
 * </ul>
 * <p>
 * This object is not associated to a file system, in other words, it only contains “static”
 * information and it may represent something that exists in no file system. Its checks are only
 * syntactic.
 * <p>
 * May be created using a git ref short form: this starts with refs/ or heads/ or tags/.
 * <p>
 * The string form of a git rev is "/" then the git ref then "//", or "/" then a 40 (?) characters
 * lower case number in hexadecimal form then "//".
 */
class GitRev {
  /**
   * Compares without access to the file system and including those roots that do not exist in an
   * underlying file system. This object does not implement Comparable because multiple
   * implementations of comparisons exist that could be considered natural. For example, with access
   * to a given file system, one could want to order roots by commit date.
   */
  static final Comparator<GitRev> SYNTAXIC_COMPARATOR =
      Comparator.comparingInt((GitRev r) -> r.isRef() ? 0 : 1).thenComparing(GitRev::toString);
  ...
  private static void checkPath(boolean check, String input, String reason)
  ...
  /**
   * A git rev equals another one iff they contain equal git refs, or they contain equal commit ids.
   */
  @Override
