...
/**
 * A git path with a root component and a (possibly empty) sequence of non-empty names.
 */
abstract class GitAbsolutePath extends GitPathImpl {
  ...
  /**
   * @param names may contain empty strings but must contain some non-empty strings; first non-empty
   *        string must start with a slash
   */
  static GitAbsolutePath givenRoot(GitPathRootImpl root, List<String> names) {
    ...
    final String[] more =
        names.isEmpty() ? new String[] {} : names.subList(1, names.size()).toArray(new String[] {});
  ...
  /**
   * If this path refers to a directory, returns a tree (which is the commit tree iff this path is
   * solely a root component). If this path refers to a file, returns a blob. TODO what about links?
   *
   * @return guaranteed to exist
   */
  abstract GitObject getGitObject(FollowLinksBehavior behavior)
      throws NoSuchFileException, IOException;

  /**
   * Returns a rev tree iff this path refers to a directory, and the commit tree iff this path is
   * solely a root component. TODO what about links?
   *
   * @throws NoSuchFileException
   * @throws NotDirectoryException
   * @throws IOException
   */
  abstract RevTree getRevTree(boolean followLinks)
  ...
  SeekableByteChannel newByteChannel(boolean followLinks) throws NoSuchFileException, IOException {
    final GitObject gitObject = getGitObject(followLinks ? FollowLinksBehavior.FOLLOW_ALL_LINKS
  ...
  BasicFileAttributes readAttributes(Set<LinkOption> optionsSet)
      throws NoSuchFileException, IOException {
    ...
    final GitBasicFileAttributes gitBasicFileAttributes =
        new GitBasicFileAttributes(gitObject, size);
    if (followLinks) {
  ...
  GitPathImpl readSymbolicLink() throws IOException, NoSuchFileException, NotLinkException,
