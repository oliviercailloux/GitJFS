 ...
 * <p>
 * Git relative paths are associated to a git absolute path, named “their absolute equivalent”, to
 * which they delegate most of their operations. The absolute equivalent of a relative path has the
 * main branch as root component and has the same internal path except that its internal path is
...
abstract class GitRelativePath extends GitPathImpl {
  ...
  static GitRelativePath relative(GitFileSystemImpl fs, List<String> names)
  ...
  static GitRelativePath relative(GitFileSystemImpl fs, Path internalPath) {
    ...
    final GitAbsolutePathWithInternal absolute =
        new GitAbsolutePathWithInternal(root, internalPath.toAbsolutePath());
