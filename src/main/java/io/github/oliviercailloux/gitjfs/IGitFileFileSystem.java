...
public interface IGitFileFileSystem extends IGitFileSystem {
	 ...
	 * {@link GitFileSystemProviderImpl#getFileSystem(URI)} to obtain this file
	 * system instance back; or to {@link GitFileSystemProviderImpl#getPath(URI)} to
	 * obtain the default path associated to this file system. It can also be given
	 * to {@link GitFileSystemProviderImpl#newFileSystem(URI)} to obtain a new file
	 * system that obtains its data by reading from the same git directory, in a new
	 * VM instance or after this one has been closed. (This identifier should not
